...
export default {
  ...
  async countProductbyCategory(catid: string | number | Readonly<any> | null){
    const list = await db("products").join("categories AS cat",{'products.catId':"cat.catId"}).where("cat.catId",catid).count({amount:"proId"});
    return list[0].amount;
  },
  async findProductbyCategoryPaging(catid: string | number | Readonly<any> | null ,offset: number,limit: number): Promise<any[]>{
    return db("products").join("categories AS cat",{'products.catId':"cat.catId"}).where("cat.catId",catid).limit(limit).offset(offset);
  }
};
